...
class WavLMWrapper(
    ...
    def __init__(
        ...
        if self.pretrain_model == "wavlm":
            backbone_ref = backbone_model_path or "microsoft/wavlm-base-plus"
            ...
            backbone_ref = backbone_model_path or "microsoft/wavlm-large"
            processor_ref = processor_path or backbone_ref
            self.processor = Wav2Vec2FeatureExtractor.from_pretrained(
                processor_ref,
                local_files_only=local_files_only,
            )
            self.backbone_model = _load_wavlm_backbone(
                backbone_ref,
                local_files_only=local_files_only,
            )
...
def _load_wavlm_backbone(model_ref, local_files_only=False):
    """Load WavLM backbone without forcing network access for config-only local dirs."""

    model_ref_path = Path(model_ref)
    if model_ref_path.exists() and not any(
        (model_ref_path / name).exists()
